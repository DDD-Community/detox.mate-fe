...
| 단계            | 에이전트                     | 모델   | 비고                                       |
| --------------- | ---------------------------- | ------ | ------------------------------------------ |
| 코드베이스 조사 | `oh-my-Codex:explore`   | sonnet | 사전 조사 (Deep Interview 전)              |
| Deep Interview  | 메인 컨텍스트                | opus   | `Skill("oh-my-Codex:deep-interview")` |
| PRD 분리        | `oh-my-Codex:architect` | opus   | 의존성 그래프 + 최소 단위 분리             |
| PRD 검증        | `oh-my-Codex:critic`    | opus   | 분리 기준 적절성 검증                      |
...
- ❌ PRD 분리 판단을 직접 하지 않는다 → `Agent(subagent_type="oh-my-Codex:architect")` 호출
- ❌ PRD 검증을 직접 하지 않는다 → `Agent(subagent_type="oh-my-Codex:critic")` 호출
...
- `oh-my-Codex:architect` → `oh-my-Codex:critic` 순차 강제 (architect 완료 후 critic 검증)
- Deep Interview는 메인 컨텍스트에서 `Skill("oh-my-Codex:deep-interview")`로 수행
- 코드베이스 조사는 `Agent(subagent_type="oh-my-Codex:explore", model="sonnet")`로 사전 실행
...
1. **Skill 호출**: `Skill("oh-my-Codex:deep-interview")`로 요구사항 명확화
