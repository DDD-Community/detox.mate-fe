...
| --------------- | ------------------- | ------------------------------------------ | ------ | -------------------------- |
| 코드베이스 조사 | Agent 서브에이전트  | `oh-my-claudecode:explore`                 | sonnet | PRD별 사전 조사            |
| Deep Interview  | **TeamCreate 팀원** | `Skill("oh-my-claudecode:deep-interview")` | opus   | 유저 질문 시 메인 프록시   |
| 레퍼런스 라우팅 | **메인 컨텍스트**   | Read                                       | opus   | 레퍼런스 읽기 + 체크리스트 |
| 구현 (ralph)    | **TeamCreate 팀원** | `Skill("oh-my-claudecode:ralph")`          | opus   | 팀원이 ralph 직접 호출     |
...
- 각 PRD에 대해 `Agent(subagent_type="oh-my-claudecode:explore", model="sonnet")`로 사전 조사
...
WORKTREE_DIR=".claude/local/worktree/prd{N}"
...
> `.claude/local/`은 `.gitignore`에 포함되어 있으므로 추적되지 않는다.
...
```
Skill("oh-my-claudecode:deep-interview") 호출 with args:
...
```
Skill("oh-my-claudecode:ralph") 호출 with args:
"다음 PRD를 구현해주세요.
...
git worktree remove .claude/local/worktree/prd{N} --force
````
